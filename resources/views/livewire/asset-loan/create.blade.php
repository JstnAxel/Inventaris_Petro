    ...
    <form wire:submit.prevent="submit" class="space-y-4 mb-10">
        @foreach ($rows as $index => $row)
            <div class="flex items-center gap-4">
                ...
                <input wire:model="rows.{{ $index }}.quantity" type="number" min="1" class="w-24 border rounded p-2" />
            ...
            @error("rows.$index.asset_id") <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
            @error("rows.$index.quantity") <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
        @endforeach

        <button type="button" wire:click="addRow" class="bg-blue-500 text-white px-4 py-1 rounded">+ Tambah Asset</button> <br>
        ...
        <tbody>
            @forelse($history as $loan)
                <tr>
                    ...
                    <td class="border px-4 py-2">{{ $loan->code ?? '-' }}</td>
                </tr>
            @empty
                <tr>
